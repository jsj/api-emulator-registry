...
function resolveQuery(fieldName: string, args: Record<string, unknown>, ls: LinearStore) {
  switch (fieldName) {
    case "viewer":
      return ls.users.all()[0] ?? null;
    case "organization":
      return typeof args.id === "string" ? (ls.organizations.findOneBy("linear_id", args.id) ?? null) : (ls.organizations.all()[0] ?? null);
    case "organizations":
      return list(ls.organizations.all(), args);
    case "user":
      return typeof args.id === "string" ? (ls.users.findOneBy("linear_id", args.id) ?? null) : null;
    case "users":
      return list(ls.users.all(), args);
    case "team":
      return typeof args.id === "string" ? (ls.teams.findOneBy("linear_id", args.id) ?? null) : null;
    case "teams":
      return list(ls.teams.all(), args);
    case "workflowState":
      return typeof args.id === "string" ? (ls.workflowStates.findOneBy("linear_id", args.id) ?? null) : null;
    case "workflowStates":
      return list(ls.workflowStates.all(), args);
    case "label":
      return typeof args.id === "string" ? (ls.labels.findOneBy("linear_id", args.id) ?? null) : null;
    case "labels":
      return list(ls.labels.all(), args);
    case "project":
      return typeof args.id === "string" ? (ls.projects.findOneBy("linear_id", args.id) ?? null) : null;
    case "projects":
      return list(ls.projects.all(), args);
    case "issue":
      if (typeof args.identifier === "string") return ls.issues.findOneBy("identifier", args.identifier) ?? null;
      if (typeof args.id === "string") return ls.issues.findOneBy("linear_id", args.id) ?? null;
...
function resolveOrganization(source: LinearOrganization, fieldName: string, args: ConnectionArgs, ls: LinearStore): unknown {
  switch (fieldName) {
    case "id":
...
function resolveWorkflowState(source: LinearWorkflowState, fieldName: string, args: ConnectionArgs, ls: LinearStore): unknown {
  switch (fieldName) {
    case "id":
      return source.linear_id;
    case "createdAt":
...
function resolveLabel(source: LinearLabel, fieldName: string, args: ConnectionArgs, ls: LinearStore): unknown {
  switch (fieldName) {
    ...
    case "issues":
      return list(ls.issues.all().filter((issue) => issue.label_ids.includes(source.linear_id)), args);
    default:
      return directValue(source as unknown as Record<string, unknown>, fieldName);
  }
}
...
function resolveIssue(source: LinearIssue, fieldName: string, args: ConnectionArgs, ls: LinearStore): unknown {
  switch (fieldName) {
    ...
    case "labels":
      return list(source.label_ids.map((id) => ls.labels.findOneBy("linear_id", id)).filter((label): label is LinearLabel => Boolean(label)), args);
    default:
      return directValue(source as unknown as Record<string, unknown>, fieldName);
  }
}

function resolveObject(source: LinearSource, args: ConnectionArgs, context: LinearGraphQLContext, info: GraphQLResolveInfo): unknown {
...
export const linearFieldResolver: GraphQLFieldResolver<LinearSource | undefined, LinearGraphQLContext> = (source, args, context, info) => {
  if (info.parentType.name === "Query") return resolveQuery(info.fieldName, args, getLinearStore(context.store));
