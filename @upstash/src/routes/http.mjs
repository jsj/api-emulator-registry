...
export function registerRoutes(app, store, contract) {
  app.post('/redis', async (c) => { const next = state(store); const input = await body(c); const result = redisReply(next, input.command ?? input[0], input.args ?? input.slice?.(1) ?? []); save(store, next); return c.json({ result }); });
