...
const workers = await request('GET', '/ccx/api/v1/emulator/workers');
assert.equal(workers.payload.data[0].descriptor, 'Ada Lovelace');
const created = await request('POST', '/ccx/api/v1/emulator/workers', { descriptor: 'Grace Hopper', primaryWorkEmail: 'grace@example.com' });
assert.equal(created.status, 201);
const fetched = await request('GET', `/ccx/api/v1/emulator/workers/${created.payload.id}`);
assert.equal(fetched.payload.descriptor, 'Grace Hopper');

console.log('workday smoke ok');
