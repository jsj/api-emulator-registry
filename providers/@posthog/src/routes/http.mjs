...
function saveEvent(store, event) {
  ...
  const existing = next.events.find((candidate) => candidate.uuid === event.uuid);
...
function hasValidApiKey(next, body, c) {
  ...
  return Boolean(key) && next.apiKeys.includes(key);
}
...
function rateLimit(next, surface, c) {
  const retryAfter = next.rateLimits[surface];
  if (!retryAfter) return null;
  return jsonWithHeaders(c, { type: 'rate_limit_error', detail: 'Rate limit exceeded' }, 429, {
    'retry-after': String(retryAfter),
  });
}

function validateEvent(body) {
  if (!body || typeof body !== 'object') return 'event must be an object';
  if (!body.event || typeof body.event !== 'string') return 'event is required';
  return null;
}
...
export function registerRoutes(app, store, contract) {
  app.post('/capture', async (c) => {
    ...
    const limited = rateLimit(next, 'capture', c);
    if (limited) return limited;
    const validationError = validateEvent(body);
    if (validationError) return c.json({ type: 'validation_error', detail: validationError }, 400);
    ...
    const result = saveEvent(store, event);
    return c.json({ status: 1, event_uuid: result.event.uuid, duplicate: result.duplicate });
  });
  app.post('/capture/', async (c) => {
    const body = await parseBody(c);
    const next = state(store);
  ...
  app.post('/batch', async (c) => {
    ...
    const limited = rateLimit(next, 'batch', c);
    if (limited) return limited;
    const invalidIndex = batch.findIndex((item) => validateEvent(item));
    if (invalidIndex !== -1) return c.json({ type: 'validation_error', detail: `batch[${invalidIndex}]: ${validateEvent(batch[invalidIndex])}` }, 400);
    const captured = batch.map((item) => {
      const event = normalizeEvent(item, 'batch');
      return saveEvent(store, event);
    });
    return c.json({ status: 1, captured: captured.filter((item) => !item.duplicate).length, duplicates: captured.filter((item) => item.duplicate).length });
  });
  app.post('/batch/', async (c) => {
    const body = await parseBody(c);
    const next = state(store);
  ...
  app.post('/control/rate-limits', async (c) => {
    const next = state(store);
    next.rateLimits = await parseBody(c);
    saveState(store, next);
    return c.json({ ok: true, rateLimits: next.rateLimits });
  });
