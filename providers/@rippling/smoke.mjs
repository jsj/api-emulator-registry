...
const employees = await request('GET', '/platform/api/employees');
assert.equal(employees.payload.data[0].work_email, 'ada@example.com');
const created = await request('POST', '/platform/api/employees', { work_email: 'grace@example.com', first_name: 'Grace', last_name: 'Hopper' });
assert.equal(created.status, 201);
const fetched = await request('GET', `/platform/api/employees/${created.payload.data.id}`);
assert.equal(fetched.payload.data.first_name, 'Grace');

console.log('rippling smoke ok');
